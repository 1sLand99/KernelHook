...
test_avd() {
    ...
    # Push files
    if [ "$effective_mode" = "sdk" ]; then
        # Push the generic kernelhook.ko plus all four ksymtab/kcrctab
        # variants. kmod_loader auto-selects the matching one via
        # /proc/kallsyms when the path argument ends in "kernelhook.ko":
        #   ksymtab=12B               → prel32               (GKI 6.1+)
        #   ksymtab=16B + kcrctab=4B  → abs64-legacy-u32     (Android 4.14+/4.19/5.0-5.3)
        #   ksymtab=16B + kcrctab=8B  → abs64-legacy         (Android 9 / 4.4)
    ...
    # Primary success signal: loader printed "loaded". Secondary signal (for
    # freestanding mode, where long init runs can time out the loader's
