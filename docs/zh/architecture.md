...
**dmesg 快照** —— 唯一的运行时观测通道。`kh_strategy_dump()` 为每一条
(capability, strategy) 元组输出一行：

```
[kh_strategy] <cap> <strategy> prio=<N> enabled=<0|1> winner=<Y|>
```

这个 dump 由 `kh_strategy_post_init()` 和 `kh_test_init()` 尾部自动调用，因此
模块加载后 `dmesg` 里总带有最新快照。`scripts/lib/strategy_matrix.sh` 以这些
行作为注册表状态的唯一来源。

> 为什么不保留 debugfs？暴露 `struct file_operations` 回调意味着 `.read` /
> `.write` 指针成为 kernel → 模块的间接调用入口点。在 kCFI 内核（GKI 6.1+）上
> 每个入口点前 4 字节必须有精确的 type hash，只能靠 `kmod_loader` 在 insmod
> 时从 vendor 参考 `.ko` 扒出来 patch。为了让一份 `kernelhook.ko` 覆盖
> 4.4–6.12，我们消除了所有非必要的 kernel → ko 入口（目前只剩
> `init_module` / `cleanup_module`，它们的 hash 由 `kmod_loader` 运行时 patch）。
> 运行时改策略改为 `rmmod + insmod <params>`，不再依赖可写的 debugfs 文件。
...
2. 将所有成功策略的返回值互相比对。
3. 任何不一致触发 `WARN` + `add_taint(TAINT_CRAP)`，并向 dmesg 输出
   `[kh_strategy] consistency mismatch in <cap>: <strategy> diverged`，由
   golden-matrix 校验器解析。
...
`kh_consistency_check=1` 的 `kernelhook.ko`，解析 `kh_strategy_post_init()`
发出的 dmesg 快照（`[kh_strategy] <cap> <strat> prio=N enabled=N winner=Y|`），
...
| `src/kh_strategy.c` | 注册表实现、模块参数 CSV 解析、`kh_strategy_dump()` |
