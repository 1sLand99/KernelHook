...
````
src/platform/syscall.c:5: * Syscall-level kh_hook infrastructure — port of KernelPatch
...
from KP `kernel/base/kh_hook.c` (the RCU-snapshot transit dispatch pattern) but that
...
| 4.5 | `kh_sys_call_table` resolved but never written through; `kh_hook_syscalln` uses inline kh_hook exclusively | KP's `hook_syscalln` prefers `fp_wrap_syscalln` when `sys_call_table` is present. | **no-action** | Documented in CLAUDE.md "Syscall hooks" section and in the file header: `sys_call_table` is `__ro_after_init` on GKI ≥ 5.10 and kCFI in `invoke_syscall+0x50` rejects the fp-kh_hook trampoline. `kh_sys_call_table` exported for diagnostic/discovery use only. Deliberate deviation from KP, justified by GKI constraints. |

### 2.3 `src/arch/arm64/transit.c` ↔ `ref/KernelPatch/kernel/base/kh_hook.c`

Diff reviewed: KP upstream (`kh_hook.c` lines 346–506 for inline transit, `fphook.c` lines
...
`hook_chain->kh_hook.relo_addr` directly across the full before+origin+after sequence, with no
critical section. KP's `kh_fp_hook_unwrap` marks items as `CHAIN_ITEM_STATE_BUSY` then
...
single unified `transit_body` + `fp_transit_body`, using a single `kh_sync_read_lock()` window
to snapshot `sorted_count`, `items[idx].{before,after,udata}`, and `rox->kh_hook.relo_addr`
...
| # | Site | Delta | Class | Rationale |
|---|------|-------|-------|-----------|
| 5.1 | RCU-snapshot design in `transit_body` / `fp_transit_body` | KP has no RCU locking at all across the entire before+origin+after sequence; our port takes a single `kh_sync_read_lock()` window, snapshots dispatch state to stack, then drops the lock before calling origin. No second lock window between origin and after-callbacks. | **no-action** | Fixes UAF: concurrent `kh_hook_unwrap_remove` + `synchronize_rcu` + `kh_mem_free_rw/rox` races against in-flight transit in KP upstream. Our deviation is stress-validated at 27.8M calls × 67K add/remove in 3 seconds, zero Oops. Reintroducing a second `kh_sync_read_lock` window between origin and after-callbacks would recreate the original bug (pre-commit e41e543). See CLAUDE.md "RCU snapshot in transit_body". |
...
| 4.5 | `src/platform/syscall.c` | **no-action** | `kh_sys_call_table` diagnostic-only; inline kh_hook path only — GKI kCFI + `__ro_after_init` make fp-kh_hook path broken; documented in CLAUDE.md | 178a698 |
