...
{
    fprintf(stderr,
        "Usage: %s --mode=<asm|asm-abs64|asm-abs64-legacy|header|symvers|self-test> "
...
/* Emit assembly populating __ksymtab_*\/__kcrctab_*\/__ksymtab_strings sections
 * in the format the Linux module loader expects (PREL32 form). Layout mirrors
 * what EXPORT_SYMBOL(...) would produce via the kernel's own macros.
 *
 * Each symbol produces three objects:
 *   1. __ksymtab_strings: null-terminated name (+ null-terminated namespace "")
 *   2. ___ksymtab+<sym>:  struct kernel_symbol { s32 value_off; s32 name_off; s32 ns_off; }
 *   3. ___kcrctab+<sym>:  s32 CRC value
 *
 * Sub-sections "___ksymtab+<sym>" let the linker sort them alphabetically when
 * collected by kmod.lds's SORT(___ksymtab+*) rule.
 */
/* Emit a GNU_PROPERTY_AARCH64_FEATURE_1_{BTI,PAC} note.  lld's -r link
 * merges `.note.gnu.property` sections with AND semantics: any input
 * object lacking a feature bit causes the output to drop it.  kh_exports.S
 * is pure asm so it doesn't inherit the note clang auto-emits for C TUs
 * — we must write it explicitly.  Payload matches the C build flag
 * `-mbranch-protection=standard` (BTI=1 + PAC=2 → 3), matching stock
 * Android common-kernel modules; Android 15 GKI 6.6+ silently rejects
 * modules whose note omits PAC. */
static void emit_note_gnu_property(FILE *out)
{
    fprintf(out,
        "\n"
...
static int emit_asm(kh_entry_t *entries, int n, FILE *out)
{
    int i;
    char canon[256];
    ...
    fprintf(out,
        ...
        "/* Generated by tools/kh_crc. DO NOT EDIT.\n"
        " * Source: kmod/exports.manifest\n"
        " * Contract 4 (algorithm freeze): CRC values are stable forever. */\n");

    emit_note_gnu_property(out);
...
/* Emit assembly populating __ksymtab with the 24-byte absolute-pointer
 * layout used on kernels built WITHOUT CONFIG_HAVE_ARCH_PREL32_RELOCATIONS
 * (e.g. Android 11 GKI 5.4 aarch64). On those kernels `struct kernel_symbol`
 * is:
 *   struct kernel_symbol {
 *       unsigned long  value;      // 8 bytes
 *       const char    *name;       // 8 bytes
 *       const char    *namespace;  // 8 bytes
 *   };
 * Total 24 bytes, all fields are absolute pointers (R_AARCH64_ABS64). The
 * 12-byte PREL32 layout produced by emit_asm() above causes strcmp crashes
 * inside find_symbol() at load time on these kernels because the kernel's
 * iterator steps 24 bytes per entry but we emit 12. See Plan 2 M-E T17+.
 * __kcrctab is unchanged (still 4-byte CRC per symbol — that layout is
 * stable across kernel versions). */
static int emit_asm_abs64(kh_entry_t *entries, int n, FILE *out)
{
    int i;
    char canon[256];
    ...
    fprintf(out,
        ...
        "/* Generated by tools/kh_crc. DO NOT EDIT.\n"
        " * Source: kmod/exports.manifest\n"
        " * Mode: asm-abs64 (24-byte kernel_symbol, non-PREL32 kernels)\n"
        " * Contract 4 (algorithm freeze): CRC values are stable forever. */\n");
...
/* Emit assembly populating __ksymtab with the 16-byte absolute-pointer
 * layout used on pre-5.3 kernels (Android 10 / 4.14, Android 9 / 4.4).
 * Those kernels have:
 *   struct kernel_symbol {
 *       unsigned long  value;      // 8 bytes
 *       const char    *name;       // 8 bytes
 *   };
 * Total 16 bytes, no namespace field (namespaces were added in upstream
 * v5.3 by commit 8651ec01daed).  Using the 24-byte asm-abs64 layout on
 * 4.14 causes strcmp(NULL) crashes in cmp_name at load time because the
 * kernel steps 16 bytes per entry while our entries are 24 bytes wide —
 * every third kernel-view entry lands on our namespace=NULL field. */
static int emit_asm_abs64_legacy(kh_entry_t *entries, int n, FILE *out)
{
    int i;
    char canon[256];
    ...
    fprintf(out,
        ...
        "/* Generated by tools/kh_crc. DO NOT EDIT.\n"
        " * Source: kmod/exports.manifest\n"
        " * Mode: asm-abs64-legacy (16-byte kernel_symbol, pre-5.3 kernels)\n"
        " * Contract 4 (algorithm freeze): CRC values are stable forever. */\n");
    ...
    /* Each entry: { value, name }. 16 bytes. No namespace field. */
    for (i = 0; i < n; i++) {
        fprintf(out,
            "\n"
            "\t.section ___ksymtab+%s,\"a\"\n"
...
/* Emit asm for Android 4.14+ / 4.19 / 5.0 .. 5.3 kernels where struct
 * kernel_symbol is still 16 bytes (no namespace field) but `__kcrctab`
 * entries were backported to u32 (commit 71810db27c1d "modversions: treat
 * symbol CRCs as 32 bit quantities" was cherry-picked into Android common
 * 4.14 branch).  Pre-Google-backport kernels (mainline 4.14, upstream 4.4,
 * Android 4.4) still use unsigned long — those must use the u64 legacy
 * variant (emit_asm_abs64_legacy).
 *
 * Layout differences from the 4.4 legacy variant:
 *   - __ksymtab:  identical (16 B, { value, name })
 *   - __kcrctab:  4 B `.long` entries instead of 8 B `.quad`.  Kernel reads
 *                  u32 per entry, so packing adjacent entries into the high
 *                  word of a .quad would contaminate the next CRC (observed
 *                  on Pixel_29 AVD — 4.14.175 Android common). */
static int emit_asm_abs64_legacy_u32(kh_entry_t *entries, int n, FILE *out)
{
    int i;
    char canon[256];
    ...
    fprintf(out,
        ...
        "/* Generated by tools/kh_crc. DO NOT EDIT.\n"
        " * Source: kmod/exports.manifest\n"
        " * Mode: asm-abs64-legacy-u32 (16 B kernel_symbol, 4 B u32 CRC)\n"
        " * Target: Android common 4.14+/4.19 + upstream 5.0..5.3\n"
        " * Contract 4 (algorithm freeze): CRC values are stable forever. */\n");
    ...
    /* __ksymtab: 16-byte entries ({ value, name }) — same as u64crc variant. */
    for (i = 0; i < n; i++) {
        fprintf(out,
            "\n"
            "\t.section ___ksymtab+%s,\"a\"\n"
    ...
    /* __kcrctab: 4-byte u32 entries — modern (post-Google-backport) layout. */
    for (i = 0; i < n; i++) {
        uint32_t crc;
        if (canonicalize(entries[i].name, entries[i].ret_tok,
                         entries[i].arg_toks,
...
int main(int argc, char **argv)
{
...

    {
        ...
        else if (strcmp(mode, "asm") == 0)     rc = emit_asm(entries, n, out);
        else if (strcmp(mode, "asm-abs64") == 0) rc = emit_asm_abs64(entries, n, out);
        else if (strcmp(mode, "asm-abs64-legacy") == 0) rc = emit_asm_abs64_legacy(entries, n, out);
        else if (strcmp(mode, "asm-abs64-legacy-u32") == 0) rc = emit_asm_abs64_legacy_u32(entries, n, out);
