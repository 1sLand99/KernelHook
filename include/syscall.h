 ...
 * non-resolvable sys_call_table is NOT treated as fatal (inline-kh_hook
 * fallback remains available), but kh_sys_call_table will be NULL.
 */
int kh_syscall_init(void);

/*
 * Install / remove a kh_hook on syscall number `nr`.
 *
 ...
 * physical kh_hook is always 1-arg (`pt_regs *`). Callers pass the
 ...
 *   physical kh_hook MUST be installed with narg==1 (arg0 = pt_regs*).
 *
...
#define kh_syscall_argn_p(args, N)                                            \
    (kh_has_syscall_wrapper                                                   \
        ? ((void *)(((uint64_t *)(uintptr_t)((kh_hook_fargs1_t *)(args))->arg0)  \
                    + (N)))                                                   \
        : ((void *)&((kh_hook_fargs8_t *)(args))->args[(N)]))
