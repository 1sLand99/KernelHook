...
/* kh_mem_ops_t.free takes (ptr) only, but kh_platform_free needs (ptr, size). */
...
{
    void *ptr = kh_platform_alloc_rox(size);
    if (ptr)
...
static void rox_free(void *ptr)
{
    ...
    if (size)
        kh_platform_free(ptr, size);
}

static int rox_set_memory(uintptr_t addr, int numpages, int (*fn)(uint64_t, uint64_t))
{
    return fn((uint64_t)addr, (uint64_t)numpages * kh_platform_page_size());
}

static int rox_set_memory_rw(uintptr_t addr, int numpages) { return rox_set_memory(addr, numpages, kh_platform_set_rw); }
static int rox_set_memory_ro(uintptr_t addr, int numpages) { return rox_set_memory(addr, numpages, kh_platform_set_ro); }
static int rox_set_memory_x(uintptr_t addr, int numpages)  { return rox_set_memory(addr, numpages, kh_platform_set_rx); }

/* ---- RW ops wrappers ---- */

static void *rw_alloc(uint64_t size)
{
    void *ptr = kh_platform_alloc_rw(size);
    if (ptr)
        alloc_track_add(ptr, size);
    return ptr;
}
...
int kh_hmem_user_init(void)
{
    ...
    return kh_mem_init(&rox_ops, &rw_ops, kh_platform_page_size());
}
