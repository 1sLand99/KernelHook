...
/* On Android static binaries, kh_platform_write_code() uses mprotect to
 * make the target's page RW.  If the target is on the same page,
 * kh_platform_write_code removes its own execute permission → SIGSEGV.
...
/* Separate targets for tests that intentionally corrupt kh_hook state */
HOOK_TARGET
...
static void misuse_before(kh_hook_fargs2_t *fargs, void *udata)
{
    (void)fargs; (void)udata;
}

/* FP target for kh_fp_unhook misuse */
HOOK_TARGET
 ...
 * Init, kh_hook a function, then call kh_hmem_user_cleanup() WITHOUT
...
TEST(misuse_cleanup_while_hooked)
{
    ...
    void *backup = NULL;
    kh_hook_err_t err = kh_hook((void *)misuse_target_cleanup, (void *)misuse_replace, &backup);
    ASSERT_EQ(err, HOOK_NO_ERR);

    /* Unhook first, THEN cleanup. On Android, cleanup without kh_unhook
     * leaves the trampoline branch patched into the function body
     * pointing at freed/unmapped ROX pages — a latent crash hazard. */
    kh_unhook((void *)misuse_target_cleanup);
    kh_hmem_user_cleanup();
}

/*
 * Test 2: misuse_double_init
 * Call kh_hmem_user_init() twice.  Either call may succeed or fail;
 * what matters is no crash.
 */
TEST(misuse_double_init)
{
    (void)kh_hmem_user_init();
    (void)kh_hmem_user_init();
    /* No crash = pass */
    kh_hmem_user_cleanup();
}
 ...
 * Init, cleanup, then try to kh_hook().  Should return an error, no crash.
 */
TEST(misuse_hook_after_cleanup)
{
    ...
    void *backup = NULL;
    kh_hook_err_t err = kh_hook((void *)misuse_target_afterclean, (void *)misuse_replace, &backup);
 ...
 * kh_hook_wrap with argno=-1.  If it succeeds, call via volatile pointer
...
TEST(misuse_wrap_argno_negative)
{
    ...
    kh_hook_err_t err = kh_hook_wrap((void *)misuse_target_neg, -1,
    ...
    if (err == HOOK_NO_ERR) {
        ...
        kh_hook_unwrap((void *)misuse_target_neg, (void *)misuse_before, NULL);
    }
    /* No crash = pass */
 ...
 * kh_hook_wrap with argno=99.  If it succeeds, call via volatile pointer,
 * then unwrap.  Cleanup.
 */
TEST(misuse_wrap_argno_overflow)
{
    ...
    kh_hook_err_t err = kh_hook_wrap((void *)misuse_target_ovf, 99,
                                   (void *)misuse_before, NULL, NULL, 0);
    if (err == HOOK_NO_ERR) {
        /* Same as argno=-1: calling with out-of-range argno reads garbage
         * stack args via the 12-arg default path. Just unwrap. */
        kh_hook_unwrap((void *)misuse_target_ovf, (void *)misuse_before, NULL);
    }
    /* No crash = pass */
 ...
 * kh_fp_hook() a target, then kh_fp_unhook() with a wrong backup pointer
 * (0xDEADBEEF).  No crash during kh_unhook = pass.
...
TEST(misuse_fp_unhook_wrong_backup)
{
    ...
    void *backup = NULL;
    kh_fp_hook((uintptr_t)&fp_misuse, (void *)misuse_replace, &backup);

    /* Pass a wrong backup — must not crash */
    void *wrong_backup = (void *)0xDEADBEEF;
    kh_fp_unhook((uintptr_t)&fp_misuse, wrong_backup);
