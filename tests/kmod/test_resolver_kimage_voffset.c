...
int test_resolver_kimage_voffset(void) {
    ...
    /* Only compare non-fallback strategies against the natural winner.
     * text_va_minus_pa is marked fallback: it derives voffset via a pgtable
     * walk which on some kernels diverges from the kernel's stored global. */
    const char *names[] = {"kallsyms", "loader_inject"};
    int i;
