...
static int __init kh_test_init(void)
{
    ...
    {
        extern int test_resolver_kimage_voffset(void);
        int rrc = test_resolver_kimage_voffset();
        tests_run++;
        if (rrc == 0) {
            tests_passed++;
            pr_info(KH_TEST_TAG "PASS: test_resolver_kimage_voffset\n");
        } else {
            tests_failed++;
            pr_err(KH_TEST_TAG "FAIL: test_resolver_kimage_voffset rc=%d\n", rrc);
        }
    }
    {
        extern int test_resolver_memstart_addr(void);
        int rrc = test_resolver_memstart_addr();
        tests_run++;
        if (rrc == 0) {
            tests_passed++;
            pr_info(KH_TEST_TAG "PASS: test_resolver_memstart_addr\n");
        } else {
            tests_failed++;
    ...
    {
        extern int test_resolver_stack(void);
        int rrc = test_resolver_stack();
        tests_run++;
        if (rrc == 0) {
            tests_passed++;
            pr_info(KH_TEST_TAG "PASS: test_resolver_stack\n");
        } else {
            tests_failed++;
            pr_err(KH_TEST_TAG "FAIL: test_resolver_stack rc=%d\n", rrc);
        }
    }
    {
        extern int test_resolver_copy_user(void);
        int rrc = test_resolver_copy_user();
        tests_run++;
        if (rrc == 0) {
            tests_passed++;
            pr_info(KH_TEST_TAG "PASS: test_resolver_copy_user\n");
        } else {
            tests_failed++;
            pr_err(KH_TEST_TAG "FAIL: test_resolver_copy_user rc=%d\n", rrc);
        }
    }
    {
        extern int test_resolver_cross_cpu_stop(void);
        int rrc = test_resolver_cross_cpu_stop();
        tests_run++;
        if (rrc == 0) {
            tests_passed++;
            pr_info(KH_TEST_TAG "PASS: test_resolver_cross_cpu_stop\n");
        } else {
            tests_failed++;
            pr_err(KH_TEST_TAG "FAIL: test_resolver_cross_cpu_stop rc=%d\n", rrc);
        }
    }
    {
        extern int test_resolver_cross_cpu_patch(void);
        int rrc = test_resolver_cross_cpu_patch();
        tests_run++;
        if (rrc == 0) {
            tests_passed++;
            pr_info(KH_TEST_TAG "PASS: test_resolver_cross_cpu_patch\n");
        } else {
            tests_failed++;
